...
- `output/amnezia-full.json` — полный GeoIP-список в формате импорта AmneziaVPN; DNS-адреса сервисов в него не входят.
- `output/amnezia-lite.json` — 1000 крупнейших российских GeoIP-сетей для iOS и Android, без DNS-адресов и без расширения маршрутов на соседние сети.
...
7. Строит полный GeoIP JSON и мобильный JSON из 1000 крупнейших GeoIP-сетей для импорта в AmneziaVPN.
...
Количество крупнейших GeoIP-маршрутов в мобильном конфиге можно изменить:

```bash
AMNEZIA_LITE_ROUTE_LIMIT=1000 npm run generate
```
...
- На Windows/macOS/Linux используйте `amnezia-full.json`: в нём находится весь точный GeoIP-набор.
- На iOS и Android используйте `amnezia-lite.json`: по умолчанию он содержит 1000 крупнейших GeoIP-сетей.
...
Оба Amnezia-конфига строятся только из `ru-geoip-cidr.txt`, поэтому ротация DNS/CDN на них не влияет. Lite-файл выбирает крупнейшие сети по количеству IPv4-адресов и сохраняет исходные CIDR без укрупнения: посторонние диапазоны не добавляются. При лимите 1000 текущая база покрывает около 68% адресного пространства полного GeoIP-набора, но небольшие российские сети, не попавшие в файл, продолжат работать через VPN.
...
- `amnezia-lite.json` намеренно охватывает не все российские сети: приоритет получают крупнейшие GeoIP-префиксы, а небольшие сети продолжают работать через VPN.
